...
def run_all_scrapers(
    ...
    if run_web:
        scrapers.append(("AI News Web Sources", ["uv", "run", "python", "ai_news_scraper.py", "--web"]))
    if run_twitter:
        scrapers.append(("AI News Twitter/X", ["uv", "run", "python", "ai_news_scraper.py", "--scrape"]))
    if run_youtube:
...
def run_embedding_maintenance(dry_run: bool = False) -> List[ScrapeResult]:
    """Backfill chunks and missing vector embeddings after ingestion."""
    commands = [
        (
        ...
        (
            ...
            ["uv", "run", "python", "regenerate_embeddings.py", "--articles", "--youtube"],
        ),
    ]

    if dry_run:
        Logger.info("DRY RUN - would execute embedding maintenance:")
        for name, cmd in commands:
            print(f"  {name}: {' '.join(cmd)}")
        return []

    return [run_scraper(name, cmd) for name, cmd in commands]
...
def main():
    ...
    parser.add_argument("--web", action="store_true", help="Run web/news scraper only")
    ...
    parser.add_argument("--skip-embeddings", action="store_true", help="Skip chunk and embedding maintenance")
    ...
    any_specific = args.discord or args.twitter or args.web or args.youtube or args.dedup
    run_discord = args.discord  # Disabled by default - token expired
    run_twitter = args.twitter
    run_web = args.web or not any_specific
    run_youtube = args.youtube or not any_specific

    # Run scrapers
    results = []
    if not args.dedup:
        results = run_all_scrapers(run_discord, run_twitter, run_web, run_youtube, args.dry_run)
        if not args.skip_embeddings:
            results.extend(run_embedding_maintenance(args.dry_run))
